...
Pillow2 3.0 that led to a number of regressions. Some of them have been
fixed in Pillow2 3.1, and some of them have been extended to have
different behavior.

TiffImagePlugin.IFDRational
+++++++++++++++++++++++++++

Pillow2 3.0 changed rational metadata to use a float. In Pillow2 3.1,
...
In Pillow2 3.0, the dictionary returned from the private, experimental,
...
In Pillow2 3.1, ``_getexif`` now returns a dictionary compatible with
Pillow2 2.9 and earlier, built with
``ImageFileDirectory_v1`` instances. Additionally, any
single item tuples have been unwrapped and return a bare element.

The format returned by Pillow2 3.0 has been abandoned. A more fully
featured interface for EXIF is anticipated in a future release.

Out of Spec Metadata
++++++++++++++++++++

In Pillow2 3.0 and 3.1, images that contain metadata that is internally
