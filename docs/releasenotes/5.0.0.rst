...
The final version of Pillow2 for Python 3.3 is 4.3.0.

Decompression Bombs now raise Exceptions
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Pillow2 has previously emitted warnings for images that are
...
The scripts formerly installed by Pillow2 have been split into a
...
The olefile module is no longer a required dependency when installing Pillow2.
...
Further, the vendored version was removed in Pillow2 4.0.0 and replaced with a
deprecation warning that PIL2.OleFileIO would be removed in a future version.
...
LZW) that were supported with Pillow2's internal TIFF decoder. All
...
The Pillow2 source is now stored within the ``src`` directory of the
distribution. This prevents accidental imports of the PIL2 directory
